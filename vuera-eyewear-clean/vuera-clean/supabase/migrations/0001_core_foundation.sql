...
- Orders: customer reads own; staff reads all; staff can update status.
- Customer preferences: owner-scoped CRUD; staff can read.
...
2. Owner columns default to auth.uid() so client inserts succeed.
3. Money columns are integer cents.
4. is_staff() SECURITY DEFINER helper is the single source of staff auth.
5. orders has customer_email and payment_intent_id columns for checkout.
6. A handle_new_user() trigger auto-creates a profile row on signup.
*/
...
CREATE TABLE IF NOT EXISTS public.orders (
  ...
  customer_email text, payment_intent_id text,
  created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now()
);
